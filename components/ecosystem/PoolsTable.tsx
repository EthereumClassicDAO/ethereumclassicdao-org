...
// Expand/collapse uses native <details>/<summary> so all rows are in the
// initial HTML (SEO-indexable) and the interaction works without JavaScript.
...
export interface PoolRow {
  ...
  type: string;
  href: string;
  /** Raw TH/s value — used to size the bar. 0 or undefined = no data. */
  hashrateTHs: number;
  /** Pre-formatted display string ("1.23 TH/s" | "456 GH/s" | "—") */
  hashrate: string;
}
...
function HashBar({ hashrateTHs, networkTHs }: { hashrateTHs: number; networkTHs: number }) {
  ...
  return (
    <div className="flex items-center justify-end gap-2">
      <div className="h-1 w-20 shrink-0 overflow-hidden rounded-full bg-[var(--divider)]">
        {pct > 0 && (
          <div
  ...
  return (
    <>
      {rows.map((pool, i) => (
        <tr
          key={pool.name}
          className={`border-b border-[var(--divider)] transition-colors hover:bg-[var(--bg-elevated)] ${
        ...
        >
          <td className="px-4 py-3">
            <a
              ...
              className="font-mono text-xs text-[var(--brand-green)] transition-colors hover:text-[var(--brand-green-hover)]"
            >
              {pool.name}
            </a>
          </td>
...
export function PoolsTable({
  ...
  const headerHashrate = networkTHs
    ? `Hashrate · ${networkTHs.toFixed(1)} TH/s`
    : "Hashrate";
  ...
  return (
    ...
    <div className="overflow-hidden rounded-xl border border-[var(--divider)]">
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-[var(--divider)] bg-[var(--bg-elevated)]">
            <th className="px-4 py-3 text-left text-xs font-mono uppercase tracking-wider text-[var(--text-subtle)]">
              Pool
            </th>
            <th className="px-4 py-3 text-left text-xs font-mono uppercase tracking-wider text-[var(--text-subtle)]">
              Fee
            ...
            <th className="px-4 py-3 text-right text-xs font-mono uppercase tracking-wider text-[var(--text-subtle)]">
              {headerHashrate}
            </th>
          </tr>
        </thead>
      ...
      {/* Remaining rows — in the DOM for SEO, collapsed by default */}
      {hasMore && (
        <details className="group">
          <summary className="flex cursor-pointer list-none items-center justify-center gap-1.5 border-t border-[var(--divider)] bg-[var(--bg-elevated)] px-4 py-3 text-xs font-mono text-[var(--text-subtle)] transition-colors hover:text-[var(--brand-green)] [&::-webkit-details-marker]:hidden">
            <ChevronDown
              size={13}
    ...
    {/* Attribution + payout type explainer */}
      ...
      <dl className="grid gap-x-6 gap-y-1.5 sm:grid-cols-2">
        {PAYOUT_TYPES.map(({ type, label, desc }) => (
          <div key={type} className="flex gap-2 text-xs leading-relaxed">
            <dt className="shrink-0 font-mono font-semibold text-[var(--brand-green)]">
              {type}
            </dt>
